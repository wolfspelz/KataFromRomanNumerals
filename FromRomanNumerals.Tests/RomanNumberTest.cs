...
namespace FromRomanNumerals.Tests
{
    ...
    {
        [TestMethod]
        public void RomanNumber_survives_the_examples_of_the_kata_description()
        {
            // Not just do the correct thing, but also explicitly care for what the customer said

            // From http://ccd-school.de/coding-dojo/function-katas/from-roman-numerals/

            Assert.AreEqual(1, RomanNumber.ToDecimal("I"));
            Assert.AreEqual(2, RomanNumber.ToDecimal("II"));
            Assert.AreEqual(4, RomanNumber.ToDecimal("IV"));
            Assert.AreEqual(5, RomanNumber.ToDecimal("V"));
            Assert.AreEqual(9, RomanNumber.ToDecimal("IX"));
            Assert.AreEqual(42, RomanNumber.ToDecimal("XLII"));
            Assert.AreEqual(99, RomanNumber.ToDecimal("XCIX"));
            Assert.AreEqual(2013, RomanNumber.ToDecimal("MMXIII"));

            int arabNumber;
            Assert.IsFalse(RomanNumber.TryToDecimal("I X", out arabNumber));
            Assert.IsFalse(RomanNumber.TryToDecimal("IC", out arabNumber));
        }

        [TestMethod]
        public void RomanNumber_can_do_all_below_100_and_other_interesting_cases()
        {
            // Arrange
        ...
        [TestMethod]
        public void RomanNumber_has_only_one_subtract_digit()
        {
            // Arrange
            int arabNumber;
            // Act // Assert
            Assert.IsTrue(RomanNumber.TryToDecimal("IX", out arabNumber));
            Assert.IsFalse(RomanNumber.TryToDecimal("IIX", out arabNumber));
            Assert.IsTrue(RomanNumber.TryToDecimal("IV", out arabNumber));
            Assert.IsFalse(RomanNumber.TryToDecimal("IIV", out arabNumber));
        }

        [TestMethod]
        public void RomanNumber_cant_aggregate_subtracts()
        {
            // Arrange
            int arabNumber;
            // Act // Assert
        ...
        [TestMethod]
        public void RomanNumber_ignores_invalid_digits()
        {
            // Arrange
            int arabNumber;
            // Act // Assert
        ...
        [TestMethod]
        public void RomanNumber_allows_only_proper_subtracts()
        {
            // Arrange
            int arabNumber;
            // Act // Assert
        ...
        [TestMethod]
        public void RomanNumber_detects_syntactically_incorrect_long_sequences_of_digits()
        {
            // Arrange
            int arabNumber;
            // Act // Assert
